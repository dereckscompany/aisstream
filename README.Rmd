...
record the raw frames now (see [`record_to_ndjson()`](#recording)) and parse them
...
The proven "read fast or get dropped" pattern: wire the recorder before connecting,
then run. It appends each raw frame to an hourly NDJSON file (rolling by UTC hour,
